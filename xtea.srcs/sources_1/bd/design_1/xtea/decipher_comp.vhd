...
architecture behavioral of decipher_comp is
	component add
...
begin
	...
	--Calculation of v0_out in 3 intermediate steps
        --Step 1
        --u_v0_inter_1 <=  ((u_v1_out sll 4) xor (u_v1_out srl 5)) + u_v1_out;
	u_v0_xor_1 <= (u_v1_out sll 4) xor (u_v1_out srl 5);
	c_v0_add0 : add
		port map(clk => clk, rst => rst, en => c_v1_add2_rdy, rdy => c_v0_add0_rdy, A => u_v0_xor_1, B => u_v1_out, Y => u_v0_inter_1);
	--Step 2
        --u_v0_inter_2 <= u_sum_inc + u_key_v0;
	c_v0_add1 : add
		port map(clk => clk, rst => rst, en => c_sum_inc_rdy , rdy => c_v0_add1_rdy, A => u_sum_inc, B => u_key_v0, Y => u_v0_inter_2);
	--Step 3
	...
	--Calculation of v1_out in 2 intermediate steps
        --Step 1
        --u_v1_inter_1 <= ((u_v0_in sll 4) xor (u_v0_in srl 5)) + u_v0_in;
        u_v1_xor_1 <= (u_v0_in sll 4) xor (u_v0_in srl 5);
	c_v1_add0 : add
		port map(clk => clk, rst => rst, en => en, rdy => c_v1_add0_rdy,  A => u_v1_xor_1, B=> u_v0_in, Y => u_v1_inter_1);
	--Step 2
        --u_v1_inter_2 <= u_sum_inc + u_key_v1;
	c_v1_add1 : add
		port map(clk => clk, rst => rst, en => en, rdy => c_v1_add1_rdy, A => unsigned(sum_in), B => u_key_v1, Y => u_v1_inter_2);
        --Step 3
	...
	-------------------------------------------------
	-- key[?] parameter
	u_key_p_v0 <= u_sum_inc and x"00000003";
	s_key_p_v0 <= std_logic_vector(u_key_p_v0(1 downto 0));
	u_key_p_v1 <= (unsigned(sum_in) srl 11) and x"00000003"; 
